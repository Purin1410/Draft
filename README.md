...
# TAMER: Tree-aware Mathematical Expression Recognition
...
## Training
Next, navigate to the TAMER folder and run `train.py`. It may take **7~8** hours on **4** NVIDIA 2080Ti gpus using ddp.
```bash
# train TAMER(Fusion) model using 4 gpus and ddp
...
# train TAMER(Self) model
cross_coverage: false
self_coverage: true

# train TAMER(Cross) model
cross_coverage: true
self_coverage: false

# train TAMER(Fusion) model
cross_coverage: true
self_coverage: true
```
...
A trained TAMER(Fusion) weight checkpoint has been saved in `lightning_logs/version_0`
