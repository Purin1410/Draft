...
class LitICAL(pl.LightningModule):
    ...
    def _compute_losses(self, batch: Batch, return_logits: bool = False):
        ...
        if return_logits:
            return loss, exp_loss, imp_loss, fusion_loss, fusion_logits
        return loss, exp_loss, imp_loss, fusion_loss

    def training_step(self, batch: Batch, batch_idx):
        loss, exp_loss, imp_loss, fusion_loss, fusion_logits = self._compute_losses(batch, return_logits=True)
        ...
        if cfg.get("enabled", False) and cfg.get("grad_norm_sample", False):
            raise NotImplementedError(
                "analysis_logging.grad_norm_sample is off by default and is not safely implemented for LitICAL."
            )

        self._analysis_log_train_batch(batch, fusion_logits, batch_idx)
    ...
    def validation_step(self, batch: Batch, _):
        ...
        self._analysis_log_batch("val", batch, fusion_logits, hyps, nbest_outputs=nbest_outputs)
    ...
    def _analysis_log_batch(self, phase: str, batch: Batch, logits_for_logging, hyps: List[Hypothesis],
        ...
        # 2. run_id resolution
        run_id = resolve_analysis_run_id(cfg, "ICAL", self.config.get("seed_everything", ""))
    ...
    def _unused_on_train_epoch_end(self):
        cfg = getattr(self, "analysis_logging_cfg", None) or {}
        if cfg.get("enabled", False) and cfg.get("merge_on_epoch_end", False):
            from utils.analysis_logging import maybe_merge_shards, resolve_analysis_run_id, get_dist_info
            run_id = resolve_analysis_run_id(cfg, "ICAL", self.config.get("seed_everything", ""))
    ...
    def test_step(self, batch: Batch, _):
        cfg = getattr(self, "analysis_logging_cfg", None) or {}
        from utils.analysis_logging import should_log_phase, should_log_topk
        ...
        if should_log_phase(cfg, "test"):
            with torch.inference_mode():
                if hasattr(batch, "exp_tgt") and hasattr(batch, "fusion_out"):
                    _, _, _, _, logits_for_logging = self._compute_losses(batch, return_logits=True)
                else:
                    exp_logits, imp_logits, fusion_logits = self(
                        batch.imgs, batch.mask, getattr(batch, "exp_tgt", getattr(batch, "tgt", None))
                    )
                    logits_for_logging = fusion_logits
    ...
    def approximate_joint_search(
        ...
        return self.ical_model.beam_search(
