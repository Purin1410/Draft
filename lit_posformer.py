...
class LitPosFormer(pl.LightningModule):
    ...
    def training_step(self, batch: Batch, batch_idx):
        ...
        if cfg.get("enabled", False) and cfg.get("grad_norm_sample", False):
            raise NotImplementedError(
                "analysis_logging.grad_norm_sample is off by default and is not safely implemented for LitPosFormer."
            )

        self._analysis_log_train_batch(batch, word_logits, batch_idx)
    ...
    def validation_step(self, batch: Batch, _):
        ...
        self._analysis_log_batch("val", batch, word_logits, hyps, nbest_outputs=nbest_outputs)
    ...
    def _maybe_log_analysis_aux(self, batch, phase: str):
        ...
        run_id = resolve_analysis_run_id(cfg, "Posformer", self.config.get("seed_everything", ""))
    ...
    def test_step(self, batch: Batch, _):
        ...
        if should_log_phase(cfg, "test"):
            with torch.inference_mode():
                out = self(batch.imgs, batch.mask, getattr(batch, "tgt", None), getattr(batch, 'pos_tgt', None))
    ...
    def approximate_joint_search(
        ...
        return self.posformer_model.beam_search(
