...
class LitTAMER(pl.LightningModule):
    ...
    def training_step(self, batch: Batch, batch_idx):
        ...
        if cfg.get("enabled", False) and cfg.get("grad_norm_sample", False):
            raise NotImplementedError(
                "analysis_logging.grad_norm_sample is off by default and is not safely implemented for LitTAMER."
            )

        self._analysis_log_train_batch(batch, output.logits, batch_idx)
    ...
    def validation_step(self, batch: Batch, _):
        ...
        self._analysis_log_batch("val", batch, output.logits, hyps, nbest_outputs=nbest_outputs)
    ...
    def _analysis_log_batch(self, phase: str, batch: Batch, logits_for_logging, hyps: List[Hypothesis],
        ...
        # 2. run_id resolution
        run_id = resolve_analysis_run_id(cfg, "TAMER", self.config.get("seed_everything", ""))
